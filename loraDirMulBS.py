    ...
    def __init__(self, id):
        ...
        if (nrBS == 4):
            if (self.id < 2):
                ...
                self.y = 2*maxY/3.0
        #self.x = random.randint(0,int(maxX)) This was Omer added line for random placement 
        #self.y = random.randint(0,int(maxY)) This was Omer added line for random placement 
...
#
class myNode():
    def __init__(self, id, period, packetlen, trafficmodel, applicationid):
        global bs
...
def transmit(env,node):
    ...
    while True:
        ...
        #yield env.timeout(86400000)
        #yield env.timeout((np.random.uniform(0, 500))*1000)
        # time sending and receiving
        # packet arrives -> add to base station
        #if noParking == 0:
           #yield env.timeout(random.expovariate(1.0/float(300000)))
           #noParking = 1
        #else:
           #yield env.timeout(random.expovariate(1.0/float(3600000)))
           #noParking = 0 
        if node.model == "-P" or node.model == "-p":
            yield env.timeout(node.interval)
            yield env.timeout(((random.randint(10, 100)))*1000)
        elif node.model == "-E" or node.model == "-e":
            yield env.timeout(random.expovariate(1.0/float(node.interval)))
        elif node.model == "-R" or node.model == "-r":
            yield env.timeout(random.randint(0, node.interval))
...
if len(sys.argv) >= 5:
    ...
    if not inputfile:
        ...
        exit(-1)
    with open(inputfile, "rb") as app:
        while True:
            parameters = app.readline() 
            ...
            if len(values) != 4:
                ...
                for i in range(0, int(values[0])):
                    node = myNode(nodeNo, int(values[2]), int(values[3]), values[1], appid)
                    nodes.append(node)
...
for i in range(0, appid+1):
    appSend.append(0)
    appReceived.append(0)

for i in range(0, nodeNo):
    appSend[nodes[i].application] = appSend[nodes[i].application] + nodes[i].sent
    appReceived[nodes[i].application] = appReceived[nodes[i].application] + nodes[i].received
...
for iterator in range (0, appid+1):
    print "DER Applicationt ", iterator+1, (float(appReceived[iterator])/float(appSend[iterator]))
